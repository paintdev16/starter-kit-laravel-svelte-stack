...
<div class="space-y-6">
    ...
    />
    {#if isLastRoot}
        <div class="space-y-4 rounded-xl border border-warning/30 bg-card-warning p-4">
            <div class="relative space-y-0.5 text-warning-foreground-soft">
                <p class="font-medium">No disponible</p>
                <p class="text-sm">Eres el único usuario root. No puedes eliminar tu cuenta.</p>
            </div>
        </div>
    {:else}
    <div
    ...
    >
        <div class="relative space-y-0.5 text-destructive">
            ...
            <p class="text-sm text-destructive-foreground-soft">Ten cuidado, esto no se puede deshacer.</p>
        </div>
        <Dialog>
            <DialogTrigger>
