<script lang="ts">
    ...
    async function save() {
        ...
        try {
            const url = editing
                ? `/admin/oauth-providers/${editing.id}`
    ...
    async function toggleShowOnLogin(p: ProviderItem) {
        try {
            const res = await fetch(
                `/admin/oauth-providers/${p.id}/show-on-login`,
                {
                    method: 'PATCH',
                    headers: {
                        Accept: 'application/json',
            ...
            if (res.ok) {
                ...
                providers = providers.map((pr) =>
                    pr.id === p.id
                        ? { ...pr, show_on_login: data.show_on_login }
                        : pr,
                );
            }
        } catch {
...
<div class="space-y-6">
    ...
    {:else if providers.length === 0}
        <div
            class="flex flex-col items-center justify-center gap-4 rounded-xl border bg-card py-16"
        >
        ...
        <div class="space-y-2">
            {#each providers as p (p.id)}
                ...
                >
                    ...
                    <Switch
                        checked={p.show_on_login}
                        onCheckedChange={() => toggleShowOnLogin(p)}
                        size="sm"
                        variant="primary"
    ...
    <DialogContent class="sm:max-w-lg">
        <DialogHeader>
            <DialogTitle
                >{editing
                    ? 'Editar proveedor'
                    : 'Añadir proveedor'}</DialogTitle
            >
            <DialogDescription>
                {editing
                    ? 'Modifica las credenciales del proveedor OAuth.'
        ...
        <div class="space-y-4 py-2">
            <div class="space-y-2">
                ...
                />
                {#if formErrors.provider}<p class="text-xs text-destructive">
                        {formErrors.provider}
                    </p>{/if}
            </div>
            <div class="space-y-2">
                ...
                />
                {#if formErrors.client_id}<p class="text-xs text-destructive">
            ...
            <div class="flex items-center gap-2">
                ...
                <Label for="show-on-login" class="text-sm font-normal"
                    >Mostrar en la pantalla de login</Label
                >
            </div>
